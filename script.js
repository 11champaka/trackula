...
// This function helps to add items to cart
...
// This function helps to multiply the quantity and the calories
function totalCost(event){
    let quantity = event.target
    quantity_parent = quantity.parentElement.parentElement
    price_field = quantity_parent.getElementsByClassName('item-price')[0]
    total_field = quantity_parent.getElementsByClassName('total-price')[0]
    price_field_content = price_field.innerText.replace('Cal ', '')
    total_field.children[0].innerText = 'Cal ' +  quantity.value * price_field_content
    grandTotal()
...
function grandTotal(){
    ...
    for(let i = 0; i < all_total_fields.length; i++){
        all_prices = Number(all_total_fields[i].innerText.replace('Cal', ''))
        total+=all_prices
    }
    grand_total.children[0].innerText = "Cal "+total
